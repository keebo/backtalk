...
**Built-in (Kokoro), the free one.** Local, offline, no accounts, no per-word costs, and honestly a bit computer-sounding. The default voice is `bm_lewis`, a British male with exactly the butler register. Around 60 voices ship free; set `voice` in `backtalk.json` (the first letter picks the language: `a` is American, `b` is British, and there are Spanish, French, Hindi, Italian, Japanese, Portuguese, and Chinese voices too). First run downloads the model from Hugging Face; anonymously that's rate-limited, so you may see an "unauthenticated requests" warning. Optional fix: grab a free, read-only token from huggingface.co (account settings → Access Tokens) and either seed it into the Keychain on macOS (`security add-generic-password -a "$USER" -s backtalk-hf-token -T /usr/bin/security -w`) or export it as `HF_TOKEN` on any OS. No token at all still works fine — you just keep the warning.
